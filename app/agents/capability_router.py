...
def infer_query_with_llm(query: str, context: dict[str, Any] | None = None) -> QueryInterpretation:
    ...
    system_prompt = (
        ...
        "1. If the user describes a symptom, condition, or diagnosis (e.g. 'chest pain', 'diabetes', 'migraine') "
        "AND mentions or implies an insurance plan AND mentions a location → route to specialist_recommendation. "
        "Examples: 'I have chest pain under Aetna plan in Dallas', 'suggest cardiologists for hypertension near Austin with BlueCross'.\n"
        "2. If the user asks for alternatives to a previously recommended provider → route to alternative_provider_suggestion.\n"
        "3. If the user explicitly asks to assess urgency or priority of a referral → route to referral_triage.\n"
        "4. If the user asks whether a provider is in-network or about insurance coverage/eligibility → route to insurance_validation.\n"
        "5. If the user asks for a general provider search by specialty or location WITHOUT a diagnosis driving the need "
        "(e.g. 'find all cardiologists in Dallas', 'list providers near me') → route to provider_discovery.\n"
        "6. If unclear, return capability='unknown'.\n\n"
